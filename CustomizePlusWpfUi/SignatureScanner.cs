...
namespace CustomizePlusWpfUi
{
	...
	public sealed class SignatureScanner
	{
		...
		public SignatureScanner(ProcessModule module, MemoryService memory)
		{
			this.memory = memory;
			this.Module = module;
		...
		public IntPtr GetStaticAddressFromSig(string signature, int offset = 0)
		{
			...
			do
			{
				...
				num = this.ReadInt32(instrAddr) + (long)instrAddr + 4 - bAddr;
				steps--;
			}
			while (steps > 0 && !(num >= this.DataSectionOffset && num <= this.DataSectionOffset + this.DataSectionSize));
			return IntPtr.Add(instrAddr, this.ReadInt32(instrAddr) + 4);
		}
		...
		public IntPtr Scan(IntPtr baseAddress, int size, string signature)
		{
			...
			this.memory.Read(baseAddress, bigBuffer, size);
		...
		private byte ReadByte(IntPtr baseAddress, int offset = 0)
		{
			...
			this.memory.Read(baseAddress + offset, buffer, 1);
			return buffer[0];
		}
		...
		private int ReadInt32(IntPtr baseAddress, int offset = 0)
		{
			...
			this.memory.Read(baseAddress + offset, buffer, 4);
			return BitConverter.ToInt32(buffer);
		}

		private short ReadInt16(IntPtr baseAddress, int offset = 0)
		{
			byte[] buffer = new byte[2];
			this.memory.Read(baseAddress + offset, buffer, 2);
			return BitConverter.ToInt16(buffer);
		}

		private long ReadInt64(IntPtr baseAddress, int offset = 0)
		{
			byte[] buffer = new byte[8];
			this.memory.Read(baseAddress + offset, buffer, 8);
		...
		private void SetupSearchSpace(ProcessModule module)
		{
			...
			int ntNewOffset = this.ReadInt32(baseAddress, 0x3C);
			IntPtr ntHeader = baseAddress + ntNewOffset;

			// IMAGE_NT_HEADER
			IntPtr fileHeader = ntHeader + 4;
			short numSections = this.ReadInt16(ntHeader, 6);
			...
			{
				long sectionName = this.ReadInt64(sectionCursor);

				// .text
				switch (sectionName)
				{
					case 0x747865742E: // .text
						this.TextSectionOffset = this.ReadInt32(sectionCursor, 12);
						this.TextSectionSize = this.ReadInt32(sectionCursor, 8);
						break;
					case 0x617461642E: // .data
						this.DataSectionOffset = this.ReadInt32(sectionCursor, 12);
						this.DataSectionSize = this.ReadInt32(sectionCursor, 8);
						break;
				}
		...
		{
			int jumpOffset = this.ReadInt32(IntPtr.Add(sigLocation, 1));
